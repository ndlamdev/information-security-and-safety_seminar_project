...
import main.java.config.AlgorithmConfig;
...
public class SelectCipherAlgorithmSymmetricalComponent extends JPanel implements Observer {
    private final int RADIUS = 20;
    private final int STROKE_WIDTH = 2;
    private AlgorithmConfig algorithmConfig;
    private JComboBox<String> jcbAlgorithm, jcbMode, jcbPadding;
    private final Function<Algorithm, Void> onAlgorithmChanged;
    ...
    public SelectCipherAlgorithmSymmetricalComponent(Function<Algorithm, Void> onAlgorithmChanged) {
        this.onAlgorithmChanged = onAlgorithmChanged;
        algorithmConfig = AlgorithmConfig.getInstance();
    ...
    private void init() {
        ...
        var algorithms = algorithmConfig.getAlgorithmSymmetrical().stream().map(AlgorithmConfig.Algorithm::alg).toArray(String[]::new);
        jcbAlgorithm = new JComboBox<>(algorithms) {{
            addActionListener(actionEvent -> {
                var alg = (String) jcbAlgorithm.getSelectedItem();
                var optional = algorithmConfig.getAlgorithmSymmetrical().stream().filter(it -> it.alg().equals(alg)).findFirst();
        ...
        var defaultAlg = algorithmConfig.getAlgorithmSymmetrical().get(0);
