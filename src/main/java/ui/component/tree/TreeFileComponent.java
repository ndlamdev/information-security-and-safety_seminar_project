...
package main.java.ui.component.tree;
...
public class TreeFileComponent extends JScrollPane {
    ...
    public void setPath(String path) {
        this.path = path;
        tree.setModel(new DefaultTreeModel(setPathHelper(path)));
    }

    private DefaultMutableTreeNode setPathHelper(String path) {
        var file = new File(path);
        DefaultMutableTreeNode root = new DefaultMutableTreeNode(file.getName());
        Arrays.stream(Objects.requireNonNull(file.listFiles(File::isFile))).forEach(it -> root.add(new DefaultMutableTreeNode(it.getName())));
        Arrays.stream(Objects.requireNonNull(file.listFiles(File::isDirectory))).forEach(it -> root.add(setPathHelper(it.getAbsolutePath())));
        return root;
    }
